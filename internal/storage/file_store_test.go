...
func TestSaveAndLoadRoundTrip(t *testing.T) {
	...
	// With the new Load behavior, participants are recalculated from tunes
	// alice provided 1 tune, bob provided 1 tune
	if out.Participants["alice"] != 1 {
		t.Fatalf("expected alice to have 1 tune, got %d: %#v", out.Participants["alice"], out)
	}
	if out.Participants["bob"] != 1 {
		t.Fatalf("expected bob to have 1 tune, got %d: %#v", out.Participants["bob"], out)
	}
	if len(out.Tunes) != 2 {
