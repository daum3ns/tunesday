...
func drawNameList(names []string, highlight int, weights ...[]float64) {
	barWidth := 10
	var weightSlice []float64
	if len(weights) > 0 {
		weightSlice = weights[0]
	}

	// Calculate maxWeight for scaling
	maxWeight := 1.0
	if len(weightSlice) > 0 {
		for _, w := range weightSlice {
			if w > maxWeight {
				maxWeight = w
			}
		}
	...
	for i, n := range names {
		...
		bar := ""
		if len(weightSlice) > i {
			filled := int((weightSlice[i] / maxWeight) * float64(barWidth))
			if filled < 0 {
				filled = 0
			}
			if filled > barWidth {
